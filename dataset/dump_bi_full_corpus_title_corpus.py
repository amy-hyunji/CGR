...
def t5_construct_sp(_model, _tokenizer, emb_f):
    ...
    if args.dump_batch == 1:
        ...
        _tok_decode, _input_ids, last_hidden_state = encode_list(["<title>"], None, _model, _tokenizer)
        tokText2tokIdList[_tok_decode[0][0]].append(2) 
        tokId2tokText[2] = _tok_decode[0][0] 
        emb_f[2][:] = last_hidden_state[0][0]
        assert _input_ids[0][0] == 32100

        _tok_decode, _input_ids, last_hidden_state = encode_list(["<context>"], None, _model, _tokenizer)
        tokText2tokIdList[_tok_decode[0][0]].append(3)
        tokId2tokText[3] = _tok_decode[0][0]
        emb_f[3][:] = last_hidden_state[0][0]
        assert _input_ids[0][0] == 32101
...
def t5_construct_corpus(_model, _tokenizer, _corpus, _context, emb_f,stopwords=None):
        ...
        for elem, tok_decode, last_hidden_state in zip(iter_corpus, tok_decode_list, last_hidden_state_list):
            ...
            for _tok, _last_hidden_state in zip(tok_decode, last_hidden_state):
                if _tok == "<pad>": 
                    ...
                    _tok_list.append(cur_tokId) ## sp token 해줘야함 
                    # tokId2corpus[cur_tokId] = elem ## 
                    emb_f[cur_tokId][:] = _last_hidden_state
                    cur_tokId += 1

            # _tok_list.append(1)
            # text = [tokId2tokText[id] for id in _tok_list]
            # print("".join(text))
...
def t5_construct_group(tokText2tokIdList):
    ...
    for tokText, tokIdList in tokText2tokIdList.items():
        if tokText == "</s>":
            ...
            print(f"Found <title> and set it to 2!!!")
            tokGroupId2tokText[2] = tokText 
            tokGroupId2tokIdList[2] = tokIdList  
            for tokId in tokIdList:
                assert tokId not in tokId2tokGroupId.keys()
                tokId2tokGroupId[tokId] = 2 
        elif tokText == "<context>":
            print(f"Found <context> and set it to 3!!!")
            tokGroupId2tokText[3] = tokText 
            tokGroupId2tokIdList[3] = tokIdList  
            for tokId in tokIdList:
                assert tokId not in tokId2tokGroupId.keys()
            ...
            tokGroupId2tokIdList[tokGroupId] = tokIdList
            for tokId in tokIdList:
                assert tokId not in tokId2tokGroupId.keys(), print(tokId)
                tokId2tokGroupId[tokId] = tokGroupId
            tokGroupId += 1
    return tokGroupId2tokText, tokId2tokGroupId, tokGroupId2tokIdList
...
def construct_dataset(_dict, tokId2clusterId, split, prefix):
    ...
    return ret_dict, f"{prefix}_{args.data_name}_{split}_cluster_{args.cluster_num}.pickle"
...
def dat2pik(args):
    ...
    dim = args.dim
    cluster = 5
    total_f = np.memmap(os.path.join(save_path, f"clusterId_emb_{cluster}.dat"), dtype="float32", mode="readonly", shape=(37000000, dim))
    total_dict = {}
    token_cnt = len(total_f.keys())
    # check if token_cnt is correct
    if total_f[token_cnt].tolist().count(0) != dim:
        assert False, f"Check token_cnt"
