...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        super().__init__(config)
        print(f"@@@@@@ Loading from local!! Using Fixed one")
        self.model_dim = config.d_model
        self.fp16 = config.fp16
        self.do_test = config.do_test
        self.train_c_emb = config.train_c_emb
        self.change_lm_head = config.change_lm_head; self.change_dec = config.change_dec; self.change_enc = config.change_enc; self.original_T5 = config.original_T5
        if config.freeze_vocab_emb:
            assert not self.change_enc
    ...
    def set_contextualized_file(self, contextualized_file):
        ...
        self.dec_shared = self.set_npd_emb(contextualized_file) 
