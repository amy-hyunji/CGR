...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        super().__init__(config)
        print(f"@@@@@@ Loading from local!! Entail_T5_fix!!")
        ...
        if self.change_enc:
            ...
            if self.do_test:
               ...
               self.shared = self.set_npd_emb(config.contextualized_file)
        else:
            self.shared = nn.Embedding(config.vocab_size, config.d_model)
        ...
        if self.change_dec:
            if self.do_test:
               ...
               self.dec_shared = self.set_npd_emb(config.contextualized_file)

        # LM Head
        if self.change_lm_head:
            self.lm_head = self.set_npd_emb(config.contextualized_file)
        ...
        if config.tie_enc_dec_vocab:
            assert not (self.change_dec or self.change_lm_head or self.change_enc)
        ...
        if self.do_test: 
            ...
            self.dec_shared = self.set_dec_shared(config.contextualized_file)

        self.lm_head = nn.Linear(config.d_model, config.vocab_size, bias=False)

        if self.tie_enc_dec_vocab:
           config.tie_word_embeddings = True
           self.dec_shared = self.shared
        else:
           config.tie_word_embeddings = False
        ...
        self.decoder = T5Stack(self.decoder_config, self.dec_shared)
    ...
    def set_npd_emb(self, file):
        if file.endswith('.pickle'):
           print(f"$$$ Loading Dec Vocab File from {file}")
