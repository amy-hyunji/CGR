...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        super().__init__(config)
        print(f"@@@@@@ Loading from local!! Encoder and Decoder both uses CV + MV")
        ...
        if self.do_test and self.train_c_emb:
            ...
            self.shared = self.set_lm_head(config.contextualized_file)
            self.lm_head = self.set_lm_head(config.contextualized_file)
        ...
        encoder_config.is_encoder_decoder = False
        if self.train_c_emb:
           self.encoder = T5Stack(encoder_config, self.shared, cond=True, train_c_emb=True)
        else:
    ...
    def couple_vanilla_vocab(self):
        ...
        with torch.no_grad():
            ...
            assert dec_embeddings.weight.shape[0]-self.config.vanilla_emb_num+3 == self.config.vanilla_emb_num-3
            dec_embeddings.weight[-config.vanilla_emb_num+3:,:] = enc_embeddings.weight[3:self.config.vanilla_emb_num,:]

    def couple_total_vocab(self):
        enc_embeddings = self.get_input_embeddings()
        dec_embeddings = self.get_output_embeddings()
        assert enc_embeddings.weight.size() == dec_embeddings.weight.size()
        with torch.no_grad():
           dec_embeddings.weight[:, :] = enc_embeddings.weight[:, :]
