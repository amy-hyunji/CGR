...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if not type(self.embed_tokens) == torch.nn.Embedding:
                print(f'is not embedding: {input_ids}')
                #assert False, f'input_ids: {input_ids} // shape: {input_ids.shape}' # [3, 1]
                print(self.embed_tokens[0].shape)
                inputs_embeds = []
                for ids in input_ids:
                    inputs_embeds.append(torch.cat([self.embed_tokens[i].unsqueeze(0).unsqueeze(0) for i in ids], dim=1))
                print(f'inputs_embeds[0].shape: {inputs_embeds[0].shape}')
...
class T5Model(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        print(f'vocab size: {config.vocab_size}') # 32128

        self.total_emb = config.total_emb
        ...
        self.decoder = T5Stack(decoder_config, self.total_emb)
    ...
    def forward(
        ...
        ### TODO: calculate lm_logits
        if self.total_emb is not None:
            _output = decoder_outputs[0]
            lm_logits=torch.einsum('bod,cd->boc', _output, self.total_emb.to(_output.device))


        if not return_dict:
            return decoder_outputs + encoder_outputs

        if self.total_emb is None:
...
#     def __init__(self, config: T5Config):
#         super().__init__(config)
#         print(f"@@@@@@ Loading from grounded_T5!! Using New one")
