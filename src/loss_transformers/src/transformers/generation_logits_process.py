...
class PrefixConstrainedLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor, batch_size: int) -> torch.FloatTensor:
        mask = torch.full_like(scores, -math.inf)
        #for batch_id, beam_sent in enumerate(input_ids.view(-1, self._num_beams, input_ids.shape[-1])):
        for batch_id, beam_sent in enumerate(input_ids.view(batch_size, -1, input_ids.shape[-1])):
