    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            beam_scores = torch.zeros((batch_size, input_ids.shape[0]), dtype=torch.float, device=input_ids.device)
            beam_scores[:, 1:] = -1e9
            beam_scores = beam_scores.view((batch_size * input_ids.shape[0],))

            next_token_scores_processed = logits_processor(input_ids, next_token_scores, batch_size=1)
