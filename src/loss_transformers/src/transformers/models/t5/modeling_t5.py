...
config.update({"contexualized_emb_num": 243245})
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            '''
            # Use negloss
            if (groupId2tokId is not None) and (tokId2groupId is not None) and (self.train_c_emb == False):
                num_token = 0
                for sen_logits, sen in zip(lm_logits, labels):
                    for logits, label in zip(sen_logits, sen):
                        if label != -100:
                            gt = label.item()
                            groupId = tokId2groupId[gt]
                            tokId_list = groupId2tokId[groupId]
                            tokId_list = torch.tensor(tokId_list, device=labels.device)

                            softmax_scores = F.log_softmax(logits[tokId_list], dim=-1)
                            gt = (tokId_list == gt).nonzero()[0,0]
                            if loss_neg is None:
                                loss_neg = F.nll_loss(softmax_scores, gt)
                            else:
                                loss_neg += F.nll_loss(softmax_scores, gt)
                            num_token += 1
                loss_neg = loss_neg / num_token

            # Use softlabel in groupId
            if (groupId2tokId is not None) and (tokId2groupId is not None):
                num_token = lm_logits.shape[-1]
                soft_labels = []
                for sen in labels:
                    soft_sen = []

                    for label in sen:
                        soft_prob = torch.zeros(num_token, device=labels.device)
                        if self.fp16:
                            soft_prob = soft_prob.half()
                        ...
                        if label != -100:
                            ...
                            embs = self.lm_head[tokId_list] # embs: (len(tokId_list), 768)
                            gt_emb = self.lm_head[gt]

                            score = torch.inner(gt_emb, embs) # score: (1, len(tokId_list))
                            topk = torch.topk(score, len(tokId_list))
                            values = topk.values # values: (1, 10)
                            indices = topk.indices # indices: (1, 10)

                            if self.fp16:
                                values = values.half()
                            
                            tokId_list = torch.tensor(tokId_list, device=labels.device)
                            soft_prob[tokId_list[indices]] = nn.functional.softmax(values, dim=-1)
                        soft_sen.append(soft_prob)
                    soft_labels.append(torch.stack(soft_sen))
                labels = torch.stack(soft_labels)

            # Use candidate loss
            if self.use_candidate:
                _batch_size = decoder_input_ids.shape[0]
                output_len = decoder_input_ids.shape[1]
                num_token = lm_logits.shape[-1]

                _mask = torch.full_like(lm_logits, -math.inf)
                for batch_id in range(_batch_size):
                    seq_len = (decoder_input_ids[batch_id] == 1).nonzero()[0,0]
                    for pos in range(output_len):
                        if pos < seq_len:
                            candId = self.get_candidate(decoder_input_ids[batch_id, pos].item())
                            _mask[batch_id, pos, candId] = 0
                        else:
                            _mask[batch_id, pos] = 0
                lm_logits += _mask
            '''
