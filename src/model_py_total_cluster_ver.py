...
class T5FineTuner(T5BaseClass):
    def __init__(self, args):
        ...
        config.update({"pad_token_id": 13382})
        ...
        #self.count_tree = pickle.load(open(os.path.join(self.hparams.dataset, self.hparams.count_tree), "rb"))
        ...
        self.pad_token_id = 13382
        self.eos_token_id = 21
        self.first_step_dict = {}

        self.cnt_over = 0
        self.len_test_dataset = len(self.test_dataloader())

    def _flush_first_step_dict(self):
        self.first_step_dict = {}
    ...
    def _get_sub_tokId_from_tokIdList(self, tokIdList, score, subcnt):
        tokIdList = torch.tensor(tokIdList)
        #values, indices = torch.topk(
        #        score[tokIdList], min(len(tokIdList), min(5, subcnt)), dim=-1, largest=True, sorted=True
        #)
        values, indices = torch.topk(
                score[tokIdList], min(5, subcnt), dim=-1, largest=True, sorted=True
        )
        tokIdList = tokIdList[indices]
        return tokIdList
    ...
    def _get_tokIdList_from_nodeIdList(self, nodeIdList, score):                        
        ...
        if self.hparams.max_beam_search:
            ...
            return list(set(tokIdList))
    def _get_tokIdList_from_groupIdList(self, groupIdList, score, count_trie_dict=None):
        ...
        if self.hparams.max_beam_search:
            for groupId in groupIdList:
                ...
                tokIdList.append(max_tokId)
            return list(set(tokIdList))
        elif self.hparams.count_beam_search:
            total_subcnt = 0
            for groupId in groupIdList:
                subcnt = count_trie_dict[groupId][-1]
                total_subcnt += min(5, subcnt)
                tokIdList.extend(self._get_sub_tokId_from_tokIdList(self.groupId2tokId[groupId], score, subcnt))
            return list(set(tokIdList)), total_subcnt
    ...
    def ids_to_text(self, _generated_ids):
        ...
        for _ids in _generated_ids:
            ...
            '''
            if "<unk>" in _text:
                generated_ids.append([3,3158,94,2579,7752,14145,3,3158, dummy]) # !!! It contains unknown token!!!
                dummy += 1
            else:
    ...
    def _get_from_trie(self, input_ids, trie_dict, score):
        ...
        if self.hparams.tree_type == "groupId":
            if len(input_ids) == 0:
                ...
                if curGroupId in set(trie_dict.keys()):
            ...
            if len(input_ids) == 0:
                ...
                if input_ids[0] in set(trie_dict.keys()):
                    return self._get_from_trie(input_ids[1:], trie_dict[input_ids[0]], score)#, count_trie_dict[clusterId])
                else:
                    return []
        else:
    ...
    def _val_step(self, batch, batch_idx, return_elem=False):
        ...
        em_list, recall_list = self.ids_calculate_scores(
            generated_ids, batch["target_ids"], batch["input"], batch_idx
        )
    ...
    def _remove_pad(self, ids):
        if ids.count(0) == 1: 
            return ids
        for i, id in enumerate(ids):
            if i != 0 and id == self.pad_token_id:   
                return ids[:i]
        assert ids[-1] == self.eos_token_id
        return ids
        #assert False

    def _remove_prev_from_trie(self, trie, ids_list, count_trie):
        temp_list = []
        ...
        if self.hparams.tree_type == "groupId":
            for ids in ids_list:
                cur_dict = trie
                count_cur_dict = count_trie
                for i in range(len(ids)-1):
                    cur_dict = cur_dict[ids[i]]
                    count_cur_dict = count_cur_dict[ids[i]]
                cur_dict.pop(ids[-1])
                count_cur_dict.pop(ids[-1])
                """
                ...
                for i in range(len(ids)-1):
                    ...
                    for j in range(len(ids)+idx):
                        ...
                        count_cur_dict = count_cur_dict[int(ids[j])]
                    if len(cur_dict[_ids]) == 0:
                        cur_dict.pop(_ids)
                        count_cur_dict[-1] = count_cur_dict[-1] - count_cur_dict[_ids][-1]
                        count_cur_dict.pop(_ids)
                        """
                        if len(cur_dict.keys()) == 1:
                            assert int(list(cur_dict.keys())[0]) == -2
                            cur_dict.pop(-2)
            ...
            assert False
        return trie, count_trie

    def _ids_remove_prev_from_trie(self, trie, ids_list):
        temp_list = []
        for elem in ids_list:
            if elem in temp_list:
                continue
        ...
        ids_list = temp_list
        for i in range(len(ids_list)):
            ids_list[i][0] = self.pad_token_id
            ids_list[i] = self._remove_pad(ids_list[i])
        if self.hparams.tree_type == "groupId" or self.hparams.tree_type == "clusterId":
            for ids in ids_list:
                cur_dict = trie
                for i in range(len(ids)-1):
                    cur_dict = cur_dict[ids[i]]
    ...
    def _test_step(self, batch, batch_idx, return_elem=False):
        ...
        _count_trie_list = [copy.deepcopy(self.count_tree) for _ in range(test_num)]
        ...
        for _iter in range(self.hparams.val_beam_size*2):
            ...
            _generated_ids = self.model.generate(
                ...
                early_stopping=True,
                #tokId2groupId=self.tokId2groupId,
                #tokId2tokText=self.tokId2tokText,
                #group_count_trie=_count_trie_list,
            )
            _generated_text = self.ids_to_text(_generated_ids)
            inum = len(_generated_ids) // self.hparams.val_beam_size
            ...
            for bid, (batch_text, batch_ids) in enumerate(zip(generated_text, generated_ids)):
                ...
                _count_trie_dict = _count_trie_list[bid]
                ...
                _trie_dict, _count_trie_dict = self._remove_prev_from_trie(_trie_dict, _upper_ids, _count_trie_dict)
    ...
    def _ids_test_step(self, batch, batch_idx, return_elem=False):
        ...
        for _iter in range(self.hparams.val_beam_size*2):
            ...
            for b_texts, b_ids in zip(generated_text, generated_ids):
                for _text, _ids in zip(b_texts, b_ids):
                    g_ids = [self.tokId2groupId[el] for el in _ids]
            """
            print(f"prediction: {generated_ids}")
            print("*"*80) 
            for bid, batch_ids in enumerate(generated_ids):
                ...
                for _ids in batch_ids:
                    if _ids in unique_ids_list[bid]:
                        ...
                        if _ids in unique_ids_list[bid]:
                            assert not self.hparams.tree_type == "nodeId"
                            _upper_ids.append([self.tokId2groupId[el] for el in _ids])
                        elif len(unique_ids_list[bid]) < self.hparams.val_beam_size:
                            unique_ids_list[bid].append(_ids)
                            if self.hparams.tree_type == "groupId":
                                _upper_ids.append([self.tokId2groupId[el] for el in _ids]) 
                            elif self.hparams.tree_type == "nodeId":
                ...
                # remove from _trie_dict
                #_trie_dict, _count_trie_dict = self._remove_prev_from_trie(_trie_dict, _upper_ids, _count_trie_dict)
                _trie_dict = self._ids_remove_prev_from_trie(_trie_dict, _upper_ids)
                _trie_list[bid] = _trie_dict
        ...
        #if self.print: print(f"## UNIQUE PRED: {unique_pred[:self.hparams.val_beam_size]}")
        em_list, recall_list = self.ids_calculate_scores(
            unique_ids_list, batch["target_ids"], batch["input"], batch_idx
        )
        if return_elem:
            assert (
                len(list(test_input))
    ...
    def test_step(self, batch, batch_idx):
        ...
        ret_dict = self._ids_test_step(batch, batch_idx, return_elem=True)
