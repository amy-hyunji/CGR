...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        print(f'#'*80)            
        if past_key_value is not None:
            print(f'past_key_value[0]: {past_key_value[0].shape}')
            print(f'past_key_value[1]: {past_key_value[1].shape}')
        else:
            print(f'past_key_values is None!')
        print(f'Shape of hidden_states: {hidden_states.shape}\nShape of key: {key_states.shape}\nShape of query: {query_states.shape}')
        ...
        print(f'## Shape of scores: {scores.shape} // Shape of position_bias: {position_bias.shape}')
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, cond=False, train_c_emb=False, corpus2mean=None):
        ...
        if corpus2mean: 
            self.meanList = list(corpus2mean.values()) 
        else:
            self.meanList = None
        ...
        if self.cond:
            if inputs_embeds is None:
                ...
                if type(self.embed_tokens)==dict:
                    ...
                    input_embeds = []
                    if self.meanList is not None:
                        for bs in _input_ids:
                            _input_embeds = [self.meanList[bs[0]]]
                            _input_embeds.extend([self.embed_tokens[el] for el in bs[1:]])
                            input_embeds.append(_input_embeds)
                    else:
                        for bs in _input_ids:
                            _input_embs = [self.embed_tokens[el] for el in bs]
                            input_embeds.append(_input_embeds)
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        self.corpus2mean = config.corpus_EmbMean
        ...
        if self.do_test and self.train_c_emb: 
            ...
            self.totalEmbMean = torch.cat([torch.tensor(el).unsqueeze(0) for el in self.corpus2mean.values()], dim=0)
            if self.fp16:
        ...
        if self.train_c_emb:
            ...
            self.decoder = T5Stack(decoder_config, self.dec_shared, cond=True, corpus2mean=self.corpus2mean)
    ...
    def forward(
        ...
        if self.train_c_emb:
            ...
            if self.totalEmbMean.get_device()  != sequence_output.get_device():
                self.totalEmbMean = self.totalEmbMean.to(sequence_output.device)
            if not return_dict:
                lm_logits = torch.einsum("bod,cd->boc", sequence_output[:, 1:, :], self.lm_head) 
                first_logits = torch.einsum("bod,cd->boc", sequence_output[:, :1, :], self.totalEmbMean)
            else:
        ...
        if labels is not None:
            ...
            loss = loss_fct(lm_logits.reshape(-1, lm_logits.size(-1)), labels.reshape(-1)) #[bs*]
        ...
        if not return_dict:
            ...
            return Seq2SeqLMOutput(
                loss=loss,
                #logits=lm_logits,
                logits={"first_logits": first_logits, "lm_logits": lm_logits, 'first_output': sequence_output[:, :1, :]},
                #past_key_values=decoder_outputs.past_key_values,
                #decoder_hidden_states=decoder_outputs.hidden_states,
                #decoder_attentions=decoder_outputs.attentions,
                #cross_attentions=decoder_outputs.cross_attentions,
