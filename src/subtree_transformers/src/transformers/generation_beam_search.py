...
class BeamSearchScorer(BeamScorer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._beam_hyps_text = [set() for _ in range(batch_size)]
    ...
    def process(
        ...
        for batch_idx, (beam_hyp, beam_hyp_text) in enumerate(zip(self._beam_hyps, self._beam_hyps_text)):
            ...
            # if is_flag == False
            is_flag = 0
            for next_token, next_score in zip(next_tokens[batch_idx], next_scores[batch_idx]):
                if (next_score.item() != -np.inf) and (next_token.item() != 1):
                    is_flag += 1
                if is_flag >= self.group_size:
                    break
            if is_flag < self.group_size:
                is_flag = True
            else:
            ...
            for beam_token_rank, (next_token, next_score, next_index) in enumerate(
                ...
                #print(f"next_score: {next_score}, next_token group: {tokId2groupId[next_token.item()]}, next_index: {next_index}")
                ...
                if (eos_token_id is not None) and (next_token.item() == eos_token_id) and (next_score.item() != -np.inf):
                    ...
                    if is_beam_token_worse_than_top_num_beams and (is_flag == False):
                        continue
                ...
                if beam_idx == self.group_size - len(beam_hyp):
                    break

            #print(f"{beam_idx}, {self.group_size}, {len(beam_hyp)}")
            if beam_idx < self.group_size - len(beam_hyp):
                raise ValueError(
