...
class PrefixConstrainedLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        for batch_id, beam_sent in enumerate(input_ids.view(-1, self._num_beams, input_ids.shape[-1])):
            #batch_subcnt = 0
            for beam_id, sent in enumerate(beam_sent): # sent: prefix tokId
                #mask[batch_id * self._num_beams + beam_id, self._prefix_allowed_tokens_fn(batch_id, sent)] = 0
                #tokIdList, beam_subcnt = self._prefix_allowed_tokens_fn(batch_id, sent, scores[batch_id * self._num_beams + beam_id])
                #batch_subcnt += beam_subcnt
                #mask[batch_id * self._num_beams + beam_id, tokIdList] = 0
                mask[batch_id * self._num_beams + beam_id, self._prefix_allowed_tokens_fn(batch_id, sent, scores[batch_id * self._num_beams + beam_id])] = 0
            #total_subcnt = max(total_subcnt, batch_subcnt)

        return scores + mask#, total_subcnt
