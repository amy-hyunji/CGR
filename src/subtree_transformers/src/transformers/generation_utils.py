...
class GenerationMixin:
    ...
    def generate(
        ...
        tokId2groupId: Optional[dict] = None,
        tokId2tokText: Optional[dict] = None,
        group_count_trie: Optional[list] = None,
        **model_kwargs,
        ...
        if is_greedy_gen_mode:
            ...
            return self.beam_search(
                ...
                tokId2groupId=tokId2groupId,
                tokId2tokText=tokId2tokText,
                group_count_trie=group_count_trie,
                **model_kwargs,
            )
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        group_ids = input_ids.clone()

        this_peer_finished = False  # used by synced_gpus only

        title2subcnt = [{} for _ in range(batch_size)]
        title2curcnt = [{} for _ in range(batch_size)]
        title2poscnt = [{} for _ in range(batch_size)]
        index2title = [{} for _ in range(batch_size*num_beams)]
        ...
        while True:
            ...
            # find possible next k for topk
            possible_next_cnt = 0
            for batch_id in range(batch_size):
                batch_possible_next_cnt = 0
                for beam_id in range(num_beams):
                    batch_beam_idx = batch_id * num_beams + beam_id
                    trie = group_count_trie[batch_id]
                    is_invalid = False
                    for group_id in group_ids[batch_beam_idx]:
                        if group_id.item() not in trie.keys():
                            is_invalid = True
                            break
                        trie = trie[group_id.item()]
                    if is_invalid == True:
                        continue
                    batch_possible_next_cnt += 5 * len(list(trie.keys())) # num_cluster * count_of_next_node
                possible_next_cnt = max(possible_next_cnt, batch_possible_next_cnt)
            ...
            next_token_scores, next_tokens = torch.topk(
                next_token_scores, min(3000, possible_next_cnt), dim=1, largest=True, sorted=True
            )
            ...
            # contraint "subcnt >= num_beam" for each path
            count_next_pos = 0
            for batch_id in range(batch_size):
                for beam_id, (next_index, next_token, next_score) in enumerate(
                    zip(next_indices[batch_id], next_tokens[batch_id], next_token_scores[batch_id])
                ):
                    if next_score.item() == -np.inf:
                        break
                    
                    batch_beam_idx = batch_id * num_beams + next_index
                    if next_index not in index2title[batch_beam_idx].keys():
                        trie = group_count_trie[batch_id]
                        title = ""
                        for token_id, group_id in zip(input_ids[batch_beam_idx], group_ids[batch_beam_idx]):
                            trie = trie[group_id.item()]
                            title += tokId2tokText[token_id.item()]
                        index2title[batch_beam_idx][next_index.item()] = (title, trie)
                        title2poscnt[batch_id][title] = trie[-1]
                    else:
                        title, trie = index2title[batch_beam_idx][next_index.item()]
                    
                    next_title = title + tokId2tokText[next_token.item()]
                    next_group_id = tokId2groupId[next_token.item()]
                    title2subcnt[batch_id][next_title] = trie[next_group_id][-1]

                    if next_title not in title2curcnt[batch_id].keys():
                        title2curcnt[batch_id][next_title] = 1
                    elif title2curcnt[batch_id][next_title] < title2subcnt[batch_id][next_title]:
                        title2curcnt[batch_id][next_title] += 1
                    else:
                        next_token_scores[batch_id, beam_id] = -np.inf

                    if next_token_scores[batch_id, beam_id] != -np.inf:
                        count_next_pos += 1
                        #print(f"!!! title: {next_title} / next_score: {next_score.item()} / next_index: {next_index.item()} / next group_id: {tokId2groupId[next_token.item()]}")
            ...
            pos_subcnt = [0 for _ in range(batch_size)]
            for title, subcnt in title2curcnt[batch_id].items():
                pos_subcnt[batch_id] += subcnt
            ...
            beam_next_groupids = beam_next_tokens.clone()
            for i, next_token in enumerate(beam_next_tokens):
                beam_next_groupids[i] = tokId2groupId[next_token.item()]

            input_ids = torch.cat([input_ids[beam_idx, :], beam_next_tokens.unsqueeze(-1)], dim=-1)
            group_ids = torch.cat([group_ids[beam_idx, :], beam_next_groupids.unsqueeze(-1)], dim=-1)
